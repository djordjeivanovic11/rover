...
class TrackedObject:
    ...
    def __init__(self, object_id: int, detection: Detection2D, pose: Pose, timestamp: float):
        ...
        self.confidence_history.append(detection.results[0].score if detection.results else 0.0)
...
class PoseEstimator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('max_distance', 1.0)  # Max distance for association
        self.declare_parameter('tracking_timeout', 3.0)  # Time before object is lost
    ...
    def process_detections(self):
        ...
        for detection_idx, track_id in associations.items():
            if track_id in self.tracked_objects:
                ...
                self.tracked_objects[track_id].update(detection, pose, current_time)
        ...
        for i, (detection, pose) in enumerate(detection_poses):
            if i not in associations:
                ...
                self.next_object_id += 1
                self.tracked_objects[new_id] = TrackedObject(new_id, detection, pose, current_time)
    ...
    def extract_3d_pose(self, detection: Detection2D) -> Optional[Pose]:
        ...
        try:
            ...
            pose = Pose()
            pose.position.x = median_point[0] + transform.transform.translation.x
            pose.position.y = median_point[1] + transform.transform.translation.y
            pose.position.z = median_point[2] + transform.transform.translation.z
    ...
    def publish_tracked_poses(self, current_time: float):
        ...
        if confirmed_objects:
            self.get_logger().debug(f"📍 Published {len(confirmed_objects)} tracked object poses")
