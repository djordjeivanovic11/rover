...
image_net = None
detections_data = []
# Frame Holding; for at most 1 frame
frame_q = Queue(maxsize=1)
detections_lock = Lock()
detections_ready = Event()
latest_boxes = []
detector_alive = Event(); 
detector_alive.set()
...
def detector_worker_onnx(session, input_name, output_names, classes,
                         input_size, conf_thres, iou_thres, urc_filter=None):
    """Continuous ONNX detector: consumes frames from frame_q and updates latest_boxes."""
    global latest_boxes
    print("🔄 ONNX detector (continuous) started...")
    frame_count = 0

    while detector_alive.is_set():
        try:
            img = frame_q.get(timeout=0.1)
        except:
            continue
        ...
        # --- Postprocess to ZED CustomBoxObjectData list ---
        detections = postprocess_onnx_detections(
            outputs[0],
            scale, pads, input_size,
        ...
        # --- Debug ---
        if frame_count % 30 == 0:
            print(f"[DEBUG/ONNX] Frame {frame_count}: {len(detections)} objects")
            for det in detections[:5]:
                label_id = det.label
                name = classes[label_id] if 0 <= label_id < len(classes) else "unknown"
                print(f"    {name}: {det.probability:.2f}")
        frame_count += 1

        # --- Publish results (non-blocking) ---
        with detections_lock:
            latest_boxes = detections
            detections_ready.set()



def detector_worker_pytorch(model, classes, conf_thres, img_size):
    """Continuous PyTorch YOLO detector: consumes frames from frame_q and updates latest_boxes."""
    global latest_boxes
    print("🔄 PyTorch YOLO detector (continuous) started...")
    frame_count = 0

    while detector_alive.is_set():
        try:
            img = frame_q.get(timeout=0.1)  # BGRA/BGR
        except:
            continue

        # Ensure RGB input for YOLO
        if img.shape[2] == 4:
        ...
        results = model.predict(
            rgb, save=False, imgsz=img_size, conf=conf_thres, verbose=False
        )[0]

        # --- Convert to ZED CustomBoxObjectData list ---
        detections = []
        for det in results.boxes:
            xywh = det.xywh[0].cpu().numpy()

            obj = sl.CustomBoxObjectData()
            obj.bounding_box_2d = xywh2abcd(xywh, img.shape)  # your helper
        ...
        # --- Debug ---
        if frame_count % 30 == 0:
            print(f"[DEBUG/PyTorch] Frame {frame_count}: {len(detections)} objects")
        frame_count += 1
...
def main():
    ...
    if args.onnx:
        ...
        try:
            ...
            def detection_thread_func(): return detector_worker_onnx(
        ...
        try:
            ...
            def detection_thread_func(): return detector_worker_pytorch(
    ...
    detection_thread = Thread(
        target=detector_worker_onnx,
        args=(session, input_name, output_names, classes,
            args.img_size, args.conf_thres, args.iou_thres, urc_filter
        ),
        daemon=True
    )
    ...
    try:
        while (not use_gl) or (viewer and viewer.is_available()):
            ...
            if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
                ...
                # Non-blocking push of newest frame for the detector
                try:
                    frame_q.get_nowait()  # drop stale frame if any (keeps latency low)
                except:
                    pass
                try:
                    frame_q.put_nowait(image_net.copy())
                except:
                    pass  # queue is full momentarily; detector will pick up next frame


                # Ingest custom boxes
                if detections_ready.is_set():
                    with detections_lock:
                        boxes = latest_boxes
                        detections_ready.clear()   # consume this batch
                else:
                    boxes = None
