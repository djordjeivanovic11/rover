...
class GNSSValidator(Node):
    ...
    def __init__(self):
        ...
        self.stability_threshold = self.get_parameter('position_stability_threshold').value
        ...
        # Publishers
        self.validation_pub = self.create_publisher(Bool, '/gnss/validation_passed', 10)
        self.stability_pub = self.create_publisher(Float32, '/gnss/position_stability', 10)
        self.readiness_pub = self.create_publisher(String, '/gnss/fusion_readiness', 10)
        
        # Services
        self.create_service(Trigger, '/gnss/validate_signal', self.validate_signal_service)
        self.create_service(Trigger, '/gnss/reset_validation', self.reset_validation)
    ...
    def validate_basic_fix_quality(self, fix_data: dict) -> bool:
        ...
        if len(fix_msg.position_covariance) >= 9:
            h_cov = (fix_msg.position_covariance[0] + fix_msg.position_covariance[4]) / 2.0
    ...
    def validate_position_stability(self) -> tuple[bool, float]:
        ...
        lon_std = np.std(lons) * 111000.0 * math.cos(math.radians(np.mean(lats)))
        alt_std = np.std(alts)
        
        # Overall position stability (RMS)
        position_std = math.sqrt(lat_std**2 + lon_std**2 + alt_std**2)
        stability_score = max(0.0, 1.0 - position_std / self.stability_threshold)
    ...
    def validate_signal_consistency(self) -> bool:
        ...
        recent_statuses = [fix['status'] for fix in list(self.fix_history)[-3:]]
    ...
    def validate_signal_service(self, request, response):
        ...
        if self.validation_passed:
            response.message = f"GNSS validation passed - ready for fusion (satellites: {self.satellite_count}, HDOP: {self.hdop_value:.2f})"
        else:
            response.message = f"GNSS validation failed - not ready for fusion"
